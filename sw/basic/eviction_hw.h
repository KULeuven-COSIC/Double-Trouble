...
double evs_create(int target_index, int threshold, int wait, int ev_size, int flags) {
...
"---|----|-------------------------------|---|-----|-----|\n");
  ...
  do {    
    ...
    if (printFLAG) {
      printf("%2d | %2d | 0x%08lx 0x%016lx | %1d | %3d | %3d |\n", 
        mem_addr,
        cnt_evs,      
        eset_hw[cnt_evs],
        eset_sw[cnt_evs],
...
int evs_verify(int target_index, uint64_t* eset_sw, int threshold, int len, int printFLAG) {
  uint64_t hw_addr;
...
void evs_recipe_write(int recipe_index) {
  ...
  uint64_t mask = ((recipe_index & 0xF) << 7) | FPGA_RECIPE_WRITE;
...
int evs_recipe_read(int recipe_index) {
  ...
  uint64_t mask = ((recipe_index & 0xF) << 7) | FPGA_RECIPE_READ;
