...
new_target:
  ...
  for (t=0; t < ITERATIONS; t++){
    m=0;
    FLUSH_A(TARGET);
    SW_READ();                     // Read line from Core A
    HW_WRITE();                    // Write line from FPGA
  ...
  printf(BLUE" [+] Other core time says \t%3u (%s)\n\n"NC, timeVictim,  timeVictim >=  SW_RAM ? "RAM" :
              ...
              (timeVictim >= SW_RL2 ? "Remote L2" : "LLC")));
...
quit:
  // Shut Down
  terminate: *syncronization = -1; sleep(1); fpga_close();
